...
def handle_movement(x_pos, y_pos, frame_width, frame_height, keys, current_animation, current_frame,
    ...
    if keys[pygame.K_a] or keys[pygame.K_LEFT]:
        ...
        if not check_full_collision(new_x_pos, y_pos, frame_width, frame_height, level_layout, block_size):
            x_pos = new_x_pos
        else:
            # Ajustar a posição para fora do bloco
            while check_full_collision(x_pos - 1, y_pos, frame_width, frame_height, level_layout, block_size):
                x_pos += 1
    ...
    if is_falling:
        ...
        if not check_full_collision(x_pos, new_y_pos, frame_width, frame_height, level_layout, block_size):
            y_pos = new_y_pos
            fall_speed += gravity
        else:
            # Ajustar a posição para fora do bloco
            while check_full_collision(x_pos, y_pos + 1, frame_width, frame_height, level_layout, block_size):
                y_pos -= 1
            fall_speed = 0
            is_falling = False
            is_jumping = False
    ...
    if is_jumping or is_falling:
        if keys[pygame.K_a] or keys[pygame.K_LEFT]:
            new_x_pos = x_pos - move_speed
            if check_full_collision(new_x_pos, y_pos, frame_width, frame_height, level_layout, block_size):
